...
//         bool Take=false;
//         if(targetnums[ind]) Take=sub(nums,target-nums[ind],ind-1,dp);
...
//         vector<vector<int>>dp(target,vector<int>(n+1,-1));
//         if(sub(nums,target,n,dp)) return true;
//         return false;
...
public:
    bool sub(vector<int>& nums, int target, int ind, vector<vector<int>>& dp) {
        ...
        bool take = false;
        if (target >= nums[ind]) {
            take = sub(nums, target - nums[ind], ind - 1, dp);
        }
